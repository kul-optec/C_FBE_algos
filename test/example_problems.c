...
/* a polynomial as f */
static unsigned int f_poly_dimension;
static unsigned int f_poly_degree;
...
real_t f_poly(const real_t* input){
    ...
    {
        unsigned int exponent = f_poly_degree-1;
        real_t base = input[i];

        unsigned int j;real_t part_output=base;
...
void df_poly(const real_t* input,real_t* output){
    ...
    for (i = 0; i < f_poly_dimension; i++)
    {
        ...
        unsigned int j;output[i]=f_poly_degree*base;
        for (j = 0; j < exponent-1; j++)
        {
...
static unsigned int problem1_dimension=0;
